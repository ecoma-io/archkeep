...
describe("readPluginOptions", () => {
  ...
  it("reads the options off this plugin's entry, however the entry names it", () => {
    // Three legitimate spellings of the same plugin: the package name once it
    // resolves from the registry, an in-repo path, and that path without the
    // entry file. Requiring one would make the options invisible in the other
    // two — and invisible options mean the defaults, silently.
    for (const specifier of [
      "@ecoma-io/nx-polyglot-graph",
      "./packages/nx-polyglot-graph/index.mjs",
  ...
  it("accepts the bare-string plugin form as declaring no options", () => {
    ...
    expect(
      readPluginOptions(
        "/w",
        treeWith({ "/w/nx.json": '{"plugins":["@ecoma-io/nx-polyglot-graph"]}' }),
      ),
    ).toEqual(DEFAULT_OPTIONS);
  });
  ...
  it("tolerates a trailing slash on the workspace root", () => {
    const nxJson = JSON.stringify({
      plugins: [{ plugin: "nx-polyglot-graph", options: { boundaryConfig: "law.mjs" } }],
    });
  ...
  it("throws on a bad option in an entry that IS present", () => {
    ...
    const nxJson = JSON.stringify({
      plugins: [{ plugin: "nx-polyglot-graph", options: { tsconfigBase: "tsconfig.base.json" } }],
    });
