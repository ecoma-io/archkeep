 ...
 * sets at once.
 */
const WATCHER_REGISTRATION_ID = "nx-polyglot-graph/watched-files";
...
export function createServer({
  ...
  function refreshOptions() {
    try {
      ...
      log(`nx-polyglot-graph: ${optionsFailure}`);
    }
  }
  ...
  async function invalidateAndRepublish() {
    ...
    refreshOptions();
    log(
      `nx-polyglot-graph: ${watchedFilesFor(options).join("/")} changed; re-diagnosing open documents`,
    );
