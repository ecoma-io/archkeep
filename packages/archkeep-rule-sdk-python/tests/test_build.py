...
from archkeep_rule_sdk import build as build_module
from archkeep_rule_sdk.build import (
    CRATE_NAME,
    BuildError,
    _findings_literal,
    _rust_string,
...
class FindingsLiteralTests(unittest.TestCase):
    """The catalogue as the literal list `archkeep_rule!` takes."""
...
class ReadDeclarationTests(unittest.TestCase):
    ...
    def test_refuses_a_module_that_states_no_declaration(self):
        with TemporaryDirectory() as directory:
            ...
            with self.assertRaises(BuildError) as raised:
                read_declaration(path)
        self.assertIn("states no ARCHKEEP_RULE mapping", str(raised.exception))

    def test_refuses_a_declaration_that_is_not_a_mapping(self):
        with TemporaryDirectory() as directory:
            path = _written("ARCHKEEP_RULE = ['no-app-to-ring']\n", directory)
            with self.assertRaises(BuildError) as raised:
                read_declaration(path)
        self.assertIn("states no ARCHKEEP_RULE mapping", str(raised.exception))
