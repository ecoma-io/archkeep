# archkeep-rule-sdk (AssemblyScript)

Write a Archkeep custom architecture rule in AssemblyScript, compile it to core
...
The npm package is `@ecoma-io/archkeep-rule-sdk`; the directory carries the `-ts`
...
`~lib/@ecoma-io/archkeep-rule-sdk.ts` and fails to find it, with or without
`--path node_modules`.
...
three exports from `archkeep_rule! { … }`, and nothing this package can write
...
[the Rust SDK's copies](../archkeep-rule-sdk-rust/fixtures), and what holds them
