...
`lattice.server.path` overrides the lookup, and exists for people working on the
...
- **Lattice: Restart the boundary server** — after installing the server package
  or changing the boundary config
- **Lattice: Show the log** — every path the extension searched, and the server's
  own output
...
[Apache-2.0](../../LICENSE) — © Mai Ngọc Hóa (John Martin) and the Lattice
contributors.
