...
#
# RUN THIS IN A CONTAINER, NOT ON YOUR OWN MACHINE. `strip = true` in
# `Cargo.toml`'s release profile removes symbols, not the panic paths rustc
# records for dependencies compiled out of the local registry — so a rebuild
# here leaves `/home/<you>/.cargo/registry/...` inside the shipped bytes. The
# committed artifact carries none, because it is built where those paths do not
# exist:
#
#   docker run --rm -v "$PWD":/src:ro rust:1.96-slim bash -c '
#     cp -r /src /work && cd /work
#     rustup target add wasm32-unknown-unknown
#     cargo build --release --target wasm32-unknown-unknown \
#       --example forbidden_tag_dependency
#     cat target/wasm32-unknown-unknown/release/examples/forbidden_tag_dependency.wasm
#   ' > examples/forbidden_tag_dependency.wasm
#
# then re-record the digest with the sha256 line below.
# `../../scripts/check-artifact-hygiene.mjs` is what enforces it; the Python
# SDK's copy of this header states the sharper version of why.
#
