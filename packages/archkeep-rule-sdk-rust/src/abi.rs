...
//! `(pointer << 32) | length`, both `u32`. The `archkeep_rule!` macro writes the
...
/// host names (`../../archkeep/src/custom-rules/host.mjs`, the `trap` arm) —
/// the loud direction, rather than a pointer the host would write through into
/// nothing.
#[cfg(target_arch = "wasm32")]
#[must_use]
pub fn alloc(len: i32) -> i32 {
    let size = usize::try_from(len).expect("archkeep_alloc: the host asks a non-negative length");
    let buffer = Box::leak(vec![0_u8; size].into_boxed_slice());
    let pointer = buffer.as_mut_ptr() as usize;
    i32::try_from(pointer).expect("archkeep_alloc: a pointer past 2 GiB does not fit the ABI's i32")
}
...
/// and the length the host wrote there — which is what the `archkeep_evaluate`
...
pub unsafe fn evidence_slice<'a>(ptr: i32, len: i32) -> &'a [u8] {
    let pointer =
        usize::try_from(ptr).expect("archkeep_evaluate: a negative pointer is not a range");
    let length = usize::try_from(len).expect("archkeep_evaluate: a negative length is not a range");
    let memory_bytes = core::arch::wasm32::memory_size(0) * 65_536;
    assert!(
        pointer.saturating_add(length) <= memory_bytes,
        "archkeep_evaluate: the evidence range reaches past this module's own linear memory"
    );
