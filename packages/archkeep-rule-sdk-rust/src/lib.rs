//! Author a Archkeep custom rule in Rust, compile it to core WebAssembly, and
...
//! is decided by the engine's host (`../../archkeep/src/custom-rules/host.mjs`)
...
//!   constraint schema has exactly one owner (`../../archkeep/src/config.mjs`)
...
/// (`../../archkeep/src/custom-rules/host.mjs`): the evidence a rule receives,
...
/// host reads `archkeep_describe` before anything else exists
/// (`../../archkeep/src/custom-rules/host.mjs`, `loadCustomRule`). So the
...
/// # What it generates
///
/// - `ARCHKEEP_RULE` — the [`RuleDeclaration`] const, so tests and the harness
///   can read the same catalogue the wasm answers with.
/// - `archkeep_describe_json()` and `archkeep_evaluate_json(&[u8])` — the pure,
...
///   of it.
/// - `archkeep_alloc`, `archkeep_describe`, `archkeep_evaluate` — three of the
...
/// (`../../archkeep/src/custom-rules/host.mjs`, `needsViolation`).
#[macro_export]
macro_rules! archkeep_rule {
    (@kind model) => {
    ...
    (
        ...
        /// This artifact's declaration — the exact document `archkeep_describe`
        /// answers with, readable from a test without going through wasm.
        pub const ARCHKEEP_RULE: $crate::RuleDeclaration = $crate::RuleDeclaration {
            name: $name,
            needs: &[ $( $crate::archkeep_rule!(@kind $kind) ),* ],
        ...
        const _: () = ARCHKEEP_RULE.assert_valid();

        /// The self-description, as UTF-8 JSON text.
        pub fn archkeep_describe_json() -> ::std::string::String {
            $crate::abi::describe_json(&ARCHKEEP_RULE)
        }

        /// This rule's verdict over one evidence bundle, as UTF-8 JSON text.
        pub fn archkeep_evaluate_json(evidence: &[u8]) -> ::std::string::String {
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn archkeep_alloc(len: i32) -> i32 {
            $crate::abi::alloc(len)
        }
        ...
        #[cfg(target_arch = "wasm32")]
        #[unsafe(no_mangle)]
        pub extern "C" fn archkeep_describe() -> i64 {
            $crate::abi::pack(archkeep_describe_json().into_bytes())
        }
        ...
        #[cfg(target_arch = "wasm32")]
        #[unsafe(no_mangle)]
        pub extern "C" fn archkeep_evaluate(ptr: i32, len: i32) -> i64 {
            // SAFETY: the host writes exactly `len` bytes at the pointer
            // `archkeep_alloc` answered with, and calls this once for that
            ...
            $crate::abi::pack(archkeep_evaluate_json(evidence).into_bytes())
        }
    };
}
