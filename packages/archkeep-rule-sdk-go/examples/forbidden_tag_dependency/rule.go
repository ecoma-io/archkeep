...
// what the Rust binding's example does (../../../archkeep-rule-sdk-rust/examples/forbidden_tag_dependency.rs)
...
func evaluate(evidence *archkeeprule.Evidence) archkeeprule.Verdict {
	...
	if err := evidence.ReadParams(&declared); err != nil {
		return archkeeprule.Unknown(fmt.Sprintf(
	...
	if !carried {
		return archkeeprule.NotApplicable(fmt.Sprintf(
	...
	for _, edge := range evidence.Graph.Edges {
		...
		finding := archkeeprule.NewFinding("dependency-on-forbidden-tag", fmt.Sprintf(
...
func exempt(project *archkeeprule.Project, tags []string) bool {
...
// nothing else (../../../archkeep/src/custom-rules/host.mjs) — but Go has no way
