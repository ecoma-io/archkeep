...
// ../../../archkeep-rule-sdk-rust/fixtures/, and the verdicts pinned below are
// the ones that crate's tests/golden.rs pins. What holds the two copies to
// each other is no longer a promise: ../../../archkeep/src/conformance/
...
//
// The rule runs through archkeeprule.EvaluateJSON rather than through the built
...
import (
	...
	"github.com/ecoma-io/archkeep/packages/archkeep-rule-sdk-go/harness"
)
...
func TestTheDeclarationIsTheDocumentTheHostReads(t *testing.T) {
	...
	if answered := archkeeprule.DescribeJSON(); answered != expected {
...
func TestAnEdgeIntoTheForbiddenTagFailsAndNamesThePair(t *testing.T) {
	harness.ExpectVerdict(t, "edge-into-forbidden-tag.json", archkeeprule.EvaluateJSON, `{
		"contract": 1,
		"verdict": "fail",
		"findings": [{
...
func TestTheVerdictBytesAreTheOnesThatCrossTheAbi(t *testing.T) {
	...
	answered := archkeeprule.EvaluateJSON(harness.ReadFixture(t, "edge-into-forbidden-tag.json"))
...
func TestAWorkspaceWhoseEdgesAllClearTheTagPasses(t *testing.T) {
	harness.ExpectVerdict(t, "every-edge-clean.json", archkeeprule.EvaluateJSON,
		`{ "contract": 1, "verdict": "pass", "findings": [] }`)
}

func TestAWorkspaceWhereTheTagAppearsNowhereIsNotApplicable(t *testing.T) {
	harness.ExpectVerdict(t, "no-project-carries-the-tag.json", archkeeprule.EvaluateJSON, `{
		"contract": 1,
...
func TestARowThatNamesNoTagIsUnknownRatherThanClean(t *testing.T) {
	harness.ExpectVerdict(t, "params-without-the-tag.json", archkeeprule.EvaluateJSON, `{
		"contract": 1,
		"verdict": "unknown",
		"findings": [],
	...
	verdict := harness.Replay(t, "params-without-the-tag.json", archkeeprule.EvaluateJSON)
...
func TestAnEdgeNamingAProjectTheModelLacksIsUnknown(t *testing.T) {
	harness.ExpectVerdict(t, "edge-into-an-undeclared-project.json", archkeeprule.EvaluateJSON, `{
		"contract": 1,
		"verdict": "unknown",
		"findings": [],
...
func TestEveryFindingTheFixturesProduceIsInTheCatalogue(t *testing.T) {
	...
	for _, entry := range archkeeprule.Registered().Findings {
		declared[entry.ID] = true
	}

	seen := 0
	for _, fixture := range fixtures {
		findings, ok := harness.Replay(t, fixture, archkeeprule.EvaluateJSON)["findings"].([]any)
		if !ok {
