# archkeep (`packages/archkeep`)

Directory-scoped mechanics only — principles live in `../../AGENTS.md` at the
repository root.
Nx project name `archkeep` (tags `type-package`, `scope-nx`), published
as `@ecoma-io/archkeep`. Plain-ESM `.mjs` with no build; `typecheck`
is `tsc --noEmit` over the JSDoc (`../../AGENTS.md`, "No TypeScript here, and
why").
...
- **`nx.mjs` holds no logic.** Nx loads it on every graph computation, so what
  ...
  that only throws, guarding the misregistration `"plugins": ["@ecoma-io/archkeep"]`
...
graph --file=` for it — the only place the package resolves and spawns the
  ...
  already required all-or-nothing from `archkeep.json`.
  `src/providers/native/` implements the same `ProjectModelProvider` seam
  from `archkeep.json` plus the tracked tree, with no Nx installed, so
...
`archkeep.json` accepts a fourth spelling for the same field: the policy
...
lives on `archkeep.json`, already watched — an object spread in would become
...
Nothing that cannot enforce reports success — see the invariant in
`../../AGENTS.md` at the repository root. Concretely:
