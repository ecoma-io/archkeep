...
// `packages/archkeep/node_modules/@nx/eslint-plugin` is this workspace's own
...
//     `packages/archkeep/src/conformance/fixture.mjs` uses for its own
...
function pluginlessFixtureRoot() {
  const root = mkdtempSync(join(tmpdir(), "archkeep-eslint-config-no-plugin-"));
  created.push(root);
  return root;
}
...
function fakePluginFixtureRoot(indexBody) {
  const root = mkdtempSync(join(tmpdir(), "archkeep-eslint-config-fake-plugin-"));
  created.push(root);
