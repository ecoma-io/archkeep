...
beforeAll(() => {
  ...
  root = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-lsp-")));
  ...
  // — the same set `archkeep check` judges (`../workspace.mjs`'s
 ...
 * spawned-process machinery (the native `archkeep.json` suite below).
 */
...
describe("a real editor session against a real workspace", () => {
  it("publishes the boundary violation a Go file crosses, at the import that crosses it", async () => {
    ...
    expect(violation.source).toBe("archkeep");
  ...
  it("registers a file watcher for every file a verdict depends on", async () => {
    ...
    // carries no `archkeep.json` of its own, but the glob is still registered so
  ...
  it("serves a session when launched through a symlink, the way an installed copy is", async () => {
    ...
    expect(initialized.result.serverInfo.name).toBe("archkeep");
...
// C1 (cubic): a `archkeep.json` root, driven through the real subprocess. The
...
// `archkeep.json` itself rather than from `nx.json` (which this root does not
...
describe("a real editor session against a native archkeep.json workspace", () => {
  let nativeRoot;
  const INNER_GO = [
    "package inner",
    "",
  ...
  beforeAll(() => {
    nativeRoot = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-lsp-native-")));
  ...
  it("resolves boundaryConfig from archkeep.json and diagnoses a real violation", async () => {
  ...
  it("judges an import into a __proto__-named project exactly as into any other", async () => {
    const makeTree = (sharedName) => {
      const dir = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-lsp-proto-")));
      const boundary = [
        "export const depConstraints = [",
      ...
      write("nx.json", '{"plugins":[{"plugin":"@ecoma-io/archkeep/nx"}]}\n');
      ...
      // The file list is tracked-only (the same set `archkeep check` judges), so
  ...
  it("refuses a root that declares both nx.json and archkeep.json", async () => {
    // One project model per workspace — the same refusal the CLI makes. A
    // tree carrying both markers is a decision nobody made.
    const both = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-lsp-both-")));
    try {
      writeFileSync(join(both, "nx.json"), "{}\n", "utf8");
      writeFileSync(join(both, "archkeep.json"), "{}\n", "utf8");
      const { readWorkspaceOptions } = await import("./lsp/server.mjs");
      expect(() => readWorkspaceOptions(both)).toThrow(/declares both nx\.json and archkeep\.json/);
    } finally {
  ...
  it("diagnoses a native root whose boundaryConfig is the inline-object form", async () => {
    ...
    // are handled now — `watchedFilesFor` leans on the `archkeep.json` entry it
    ...
    // and onto `archkeep.json` — so what differs between the two is the dialect
    // and nothing else, and both must reach the identical violation.
    const inline = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-lsp-inline-")));
