 ...
 * `moon run archkeep:typecheck` failure with nothing wrong in the test.
 *
...
describe("a workspace pointing its profiles option at an installed pack", () => {
  const root = mkdtempSync(join(tmpdir(), "archkeep-presets-"));
  ...
  const installedAt = "node_modules/@ecoma-io/archkeep/presets/hexagonal.json";
...
`,
  ...
  it("is the could-not-look exit class when the pack is not where the option says", async () => {
    const detached = mkdtempSync(join(tmpdir(), "archkeep-presets-missing-"));
    try {
      writeFileSync(
