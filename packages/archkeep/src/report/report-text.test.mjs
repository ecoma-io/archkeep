...
const context = { root: "/workspace", provider: "native", marker: "archkeep.json" };
...
describe("formatGovernanceReport — the headline states the document's own verdict", () => {
  ...
  it("leads with NO VERDICT and counts the gaps when any surface could not be inspected", () => {
    const text = formatGovernanceReport(
      input({
        uninspectable: [
          ...
          { surface: "waivers", reason: "archkeep: waivers has incomplete coverage — 1 file" },
        ],
      }),
    );
    ...
    expect(text).toContain("  waivers: archkeep: waivers has incomplete coverage — 1 file");
  });
  ...
  it("prints an unknown surface with its reason, and no count beside it", () => {
    const text = formatGovernanceReport(
      input({
        waivers: {
          verdict: "unknown",
          note: "archkeep: waivers has incomplete coverage",
          counts: null,
          rows: [],
        },
        uninspectable: [
          { surface: "waivers", reason: "archkeep: waivers has incomplete coverage" },
        ],
      }),
    );
    expect(text).toContain(
      "  unknown         waivers  (archkeep: waivers has incomplete coverage)",
    );
  ...
  it("does not claim a ref names nothing when the registry could not be read", () => {
    const text = formatGovernanceReport(
      input({
        decisions: {
          verdict: "unknown",
          note: "archkeep: cannot read docs/adr: ENOTDIR",
        ...
        uninspectable: [
          { surface: "decisions", reason: "archkeep: cannot read docs/adr: ENOTDIR" },
        ],
      }),
    );
