...
beforeAll(() => {
  root = mkdtempSync(join(tmpdir(), "archkeep-index-"));
...
describe("where the file list comes from", () => {
  ...
  it("fails loudly, naming the directory, when git cannot answer for the tree", () => {
    ...
    const outside = mkdtempSync(join(tmpdir(), "archkeep-not-a-repo-"));
    try {
  ...
  it("refuses a symlink whose realpath leaves the workspace — the LSP read escape (G-10)", () => {
    ...
    const outside = mkdtempSync(join(tmpdir(), "archkeep-index-escape-"));
    try {
...
// `discover()`/`buildGraph()`, a `archkeep.json` root was still read through
...
// above — an Nx-shaped fixture and a `archkeep.json`-shaped one cannot share a
// root, because `buildWorkspaceIndex` picks its branch by which marker file
// the root carries.
describe("a native archkeep.json workspace, driven through the real provider", () => {
  let nativeRoot;
  ...
  beforeAll(() => {
    nativeRoot = mkdtempSync(join(tmpdir(), "archkeep-native-index-"));
    // Two declared projects, NEITHER with a `project.json` — the whole point
    // of `archkeep.json` is not needing one, and it is the case
    // `discoverProjects` cannot see at all.
    writeFileSync(
      join(nativeRoot, "archkeep.json"),
      JSON.stringify({
        projects: {
          declared: [
  ...
  it("flags a declared implicit dependency across a tag boundary, like check does", () => {
    ...
    const implicitRoot = mkdtempSync(join(tmpdir(), "archkeep-native-implicit-"));
    ...
    try {
      writeFileSync(
        join(implicitRoot, "archkeep.json"),
        JSON.stringify({
          projects: {
            declared: [
              {
  ...
  it("still dispatches natively when archkeep.json exists on disk but git does not track it", () => {
    // The dispatch-parity bug (S-bug C): `buildWorkspaceIndex` used to gate the
    // native branch on `files.includes(ARCHKEEP_MODEL_FILE)` — `files` being
    ...
    // `archkeep.json` — written to the tree but never `git add`ed — passes the
    // CLI's test and fails the old index's, so `archkeep check` on this exact
    ...
    const untrackedRoot = mkdtempSync(join(tmpdir(), "archkeep-native-untracked-"));
    ...
    try {
      ...
      // Every file is staged EXCEPT archkeep.json, added last and deliberately
      ...
      writeFileSync(
        join(untrackedRoot, "archkeep.json"),
        JSON.stringify({
          projects: {
            declared: [
              { root: "apps/inner", tags: ["zone:inner"] },
  ...
  it("refuses to call a document analyzed when archkeep.json itself is broken", () => {
    ...
    // reach that throw for a `archkeep.json` root — this proves it is now
    // caught into `nativeModelFailure` rather than crashing the server or
    // (the silent direction) never running the check at all.
    const brokenRoot = mkdtempSync(join(tmpdir(), "archkeep-native-broken-"));
    try {
      writeFileSync(
        join(brokenRoot, "archkeep.json"),
...
// empty diagnostic list on a tree `archkeep check` exits 1 on. The branch below
...
describe("a .moon workspace, populated through the same provider seam check holds", () => {
  ...
  beforeAll(() => {
    moonRoot = mkdtempSync(join(tmpdir(), "archkeep-moon-index-"));
  ...
  it("judges a violating import the way check does, instead of publishing clean", () => {
    ...
    // editor painting clean the file `archkeep check` exits 1 on.
  ...
  it("reports a failing provider invocation as a gap, never as a clean empty index", () => {
    ...
    const index = buildWorkspaceIndex({
      ...
      readGraph: () => {
        throw new Error(
          `archkeep: \`moon project-graph --json\` failed in ${moonRoot}: spawn moon ENOENT`,
        );
      },
    });
