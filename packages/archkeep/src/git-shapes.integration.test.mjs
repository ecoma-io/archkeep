...
describe("a linked worktree checkout", () => {
  ...
  const ARCHKEEP_MODEL = JSON.stringify({
    projects: {
      declared: [
...
`;
  ...
  const mainRoot = mkdtempSync(join(tmpdir(), "archkeep-worktree-main-"));
  const linkedRoot = mkdtempSync(join(tmpdir(), "archkeep-worktree-linked-"));
  afterAll(() => {
  ...
  const head = (() => {
    writeIn(mainRoot, "archkeep.json", ARCHKEEP_MODEL);
...
describe("a detached HEAD", () => {
  const repo = mkdtempSync(join(tmpdir(), "archkeep-detached-head-"));
...
describe("a submodule under the superproject", () => {
  ...
  const extRepo = mkdtempSync(join(tmpdir(), "archkeep-submodule-ext-"));
  const superRoot = mkdtempSync(join(tmpdir(), "archkeep-submodule-super-"));
  afterAll(() => {
