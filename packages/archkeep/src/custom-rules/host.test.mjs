...
describe("the emitted fixtures are modules Node accepts", () => {
  ...
  const variants = [
    ...
    ["a misdeclared export", { misdeclareExport: "archkeep_alloc" }],
    ...RULE_EXPORTS.map(
      (name) =>
...
describe("loadCustomRule", () => {
  ...
  it("refuses to load a rule whose description never returns, naming the budget", async () => {
    ...
    expect(loaded.failure.reason).toMatch(
      /archkeep_describe did not return within its 75ms budget and was terminated/u,
    );
  });
...
describe("evaluateCustomRule", () => {
  ...
  it("hands the rule the exact evidence bytes, at the pointer archkeep_alloc named", async () => {
...
describe("what evaluateCustomRule refuses, by name", () => {
  ...
  it("refuses when archkeep_alloc traps", async () => {
    ...
    assertRefused(
      ...
      /archkeep_alloc trapped \(unreachable\)/u,
    );
  });

  it("refuses when archkeep_evaluate traps", async () => {
    assertRefused(
      await evaluate({ evaluateBehavior: "trap" }),
      /archkeep_evaluate trapped \(unreachable\)/u,
    );
  });
  ...
  it("refuses when archkeep_alloc answers a pointer the evidence cannot be written at", async () => {
    assertRefused(
      await evaluate({ allocBehavior: "out-of-bounds" }),
      /archkeep_alloc answered 65552 for \d+ evidence bytes, which does not fit in its 65536 bytes/u,
    );
  });
  ...
  it("refuses a claimed range larger than this host will ever copy", async () => {
    ...
    assertRefused(
      result,
      new RegExp(
        `archkeep_evaluate claimed ${CUSTOM_RULE_MAX_VERDICT_BYTES + 1} bytes, past the ` +
