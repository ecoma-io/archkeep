...
describe("buildNativeGraph", () => {
  ...
  // `archkeep.json`'s declared-project row (a hypothetical `entryPoints` or
  // `mfeRemote`) straight through — fields the Nx path computes itself, from
  // real package.json/webpack facts, that a native `archkeep.json` author could
  ...
  // S5: `archkeep.json`'s `projects.declared[].targets` names target names
  ...
  it("synthesizes data.targets from a project's declared target names", () => {
    ...
    expect(nodes.a.data.targets).toEqual({
      build: { executor: "archkeep:declared" },
      test: { executor: "archkeep:declared" },
    });
  });
  ...
  // bug this pins against: `archkeep.json`'s `workspaceLayout` used to be
  ...
  // comes straight from `archkeep.json` or a tracked manifest, both
