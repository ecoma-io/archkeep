 ...
 *   `archkeep.json` marker, must reach the identical verdict; a root carrying
 ...
 *   file, an ESLint flat config, and an inline `archkeep.json` object must
...
describe("A1 — path source: the default location and --config pointing at a relocated copy", () => {
  ...
  const write = writeIn(root);
  const law = buildLaw();
  write("nx.json", JSON.stringify({ plugins: [{ plugin: "@ecoma-io/archkeep/nx", options: {} }] }));
...
describe("A2 — filename: a non-default boundaryConfig name changes nothing about the law", () => {
  ...
  writeCustom(
    "nx.json",
    JSON.stringify({
      plugins: [{ plugin: "@ecoma-io/archkeep/nx", options: { boundaryConfig: "law/custom.mjs" } }],
    }),
  );
  ...
  it("red twin — a declared filename that does not exist throws instead of silently falling back to the default name", async () => {
    ...
    try {
      ...
      write(
        ...
        JSON.stringify({
          plugins: [
            { plugin: "@ecoma-io/archkeep/nx", options: { boundaryConfig: "law/custom.mjs" } },
          ],
        }),
      );
...
// ---------------------------------------------------------------------------
// A3 — marker: nx.json vs archkeep.json
// ---------------------------------------------------------------------------

describe("A3 — marker: the nx.json route and the archkeep.json route judge the same tree identically", () => {
  ...
  it("both markers report the identical single violation", async () => {
    ...
    const exemptionNote = "; 1 file exempted from coverage by archkeep.json's coverage.exempt";
    ...
    expectReportsEqual(
      [
        ...
        {
          spelling: "archkeep.json marker (exemption note stripped)",
  ...
  it("red twin — a root carrying both markers refuses to run rather than silently picking one", async () => {
    ...
    try {
      ...
      write("archkeep.json", JSON.stringify({ projects: { declared: [] } }));
      ...
      ).rejects.toThrow(/declares both nx\.json and archkeep\.json/);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
// ---------------------------------------------------------------------------
// A4 — dialect: .mjs, .json, ESLint flat config, inline archkeep.json object
// ---------------------------------------------------------------------------

describe("A4 — dialect: the same law as .mjs, .json, an ESLint flat config, and an inline archkeep.json object", () => {
  ...
  writeEslint(
    ...
    JSON.stringify({
      plugins: [
        { plugin: "@ecoma-io/archkeep/nx", options: { boundaryConfig: "eslint.config.mjs" } },
      ],
    }),
  );
  ...
  const inlineFiles = ["archkeep.json", ...Object.keys(SIMPLE_GO_FILES)];
        ...
        },
        {
          ...
          loader: Promise.resolve(
            policyFrom(
              ...
              "inline archkeep.json boundaryConfig",
            ),
          ),
        },
  ...
  it("Tier 2 — all four dialects judge the same tree to the identical verdict", async () => {
    ...
    // (the inline object has no file of its own) `archkeep.json` itself — so
    ...
    expect(inlinePolicy.source).toBe("archkeep.json");
    ...
    expectReportsEqual(
      [
        ...
        { spelling: "inline (archkeep.json)", report: inlinePolicy.rest },
      ],
      4,
    );
  });
