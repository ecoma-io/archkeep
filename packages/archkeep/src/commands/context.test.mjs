...
describe("resolveCommandContext — the loud refusals it owns", () => {
  it("throws when the root carries both nx.json and archkeep.json, naming the tree", () => {
    ...
    expect(() =>
      resolveCommandContext(
        { cwd: root },
        { listFiles: () => ["nx.json", "archkeep.json"], readGraph: () => ({ nodes: {} }) },
      ),
    ).toThrow(/declares both nx\.json and archkeep\.json/);
  });
...
describe("resolveCommandContext — pluginGap", () => {
  it("is always {registered: true, manifests: []} on a native workspace", () => {
    ...
    const context = resolveCommandContext(
      { cwd: root },
      { listFiles: () => ["archkeep.json", "libs/a/go.mod"] },
    );
      ...
      { cwd: root },
      {
        ...
        readFile: (path) =>
          path === join(root, "nx.json")
            ? JSON.stringify({ plugins: ["@ecoma-io/archkeep/nx"] })
            : null,
      },
    );
...
describe("markersAt — detects all three workspace markers", () => {
  ...
  it("detects archkeep.json as a native marker", () => {
...
describe("resolveCommandContext — Moon ambiguity refusals", () => {
  ...
  it("throws when the root carries both .moon and archkeep.json", () => {
    ...
    expect(() =>
      resolveCommandContext(
        { cwd: root },
        { listFiles: () => [".moon/tool.yml", "archkeep.json"] },
      ),
    ).toThrow(/declares both \.moon and archkeep\.json/);
  });
...
describe("resolveCommandContext — the Moon branch scopes before it analyzes", () => {
  it("selects the Moon provider when only .moon is present", () => {
    ...
    expect(context.pluginGap).toEqual({ registered: true, manifests: [] });
    // Options come from defaults (no nx.json plugins table, no archkeep.json).
...
describe("resolveCommandContext — unownedGap, the tolerated half of the same question", () => {
  ...
  it("stays empty on a native workspace, which refuses the same state loudly instead", () => {
    ...
    const context = resolveCommandContext(
      { cwd: root },
      { listFiles: () => ["archkeep.json", "libs/a/a.go", "tooling.config.mjs"] },
    );
...
describe("resolveCommandContext — options.boundaryConfigDeclared, on all three providers", () => {
  ...
  /** The native branch, driven over a `archkeep.json` carrying `model`. */
  const nativeContextFor = (model, label) => {
    ...
    return resolveCommandContext(
      { cwd: root },
      { listFiles: () => ["archkeep.json", "libs/a/go.mod", "libs/a/a.go"] },
    );
  };
  ...
  it("native: false when archkeep.json names no law", () => {
  ...
  it("native: true when archkeep.json names a law, even the convention filename itself", () => {
  ...
  it("native: true for the inline-policy spelling, which names no file at all", () => {
    ...
    // field on `archkeep.json`, which is present by construction — so this
  ...
  it("nx: false when nx.json registers the plugin without naming a law", () => {
    const bare = nxContextFor('{"plugins":["@ecoma-io/archkeep/nx"]}\n', "bare");
  ...
  it("moon: always false, because a Moon workspace has nowhere to name a law", () => {
    ...
    // table and a `archkeep.json` beside `.moon/` is refused outright
