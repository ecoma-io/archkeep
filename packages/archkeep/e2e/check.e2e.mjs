...
describe("check", () => {
  it("exits 0 on a clean native tree and states what it inspected", () => {
    const result = archkeep(nativeConsumer.root, ["check"]);
  ...
  it("exits 0 on a clean Nx tree and states what it inspected", () => {
    const result = archkeep(nxConsumer.root, ["check"]);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toMatch(/[1-9]\d* import/);
    expect(result.stdout).toMatch(/[1-9]\d* file/);
    expect(result.stdout).toMatch(/[1-9]\d* project/);
  ...
  it("produces a valid JSON envelope on a clean native tree", () => {
    const result = archkeep(nativeConsumer.root, ["check", "--format", "json"]);
    expect(result.exitCode).toBe(0);
  ...
  it("exits 3 when run outside any workspace (no marker)", () => {
    // Running in an empty directory with no marker file and no archkeep
    // installed — `pnpm exec archkeep` fails because pnpm cannot find the
    ...
    const nowhere = mkdtempSync(join(tmpdir(), "archkeep-e2e-nowhere-"));
    try {
      const result = archkeep(nowhere, ["check"]);
