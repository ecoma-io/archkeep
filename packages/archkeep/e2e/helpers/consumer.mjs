...
// because Archkeep discovers files via `git ls-files` and a workspace with
...
function commitTree(consumer, message, fresh) {
  ...
  run(
    ...
    [
      "-c",
      "user.name=archkeep-e2e",
      "-c",
      "user.email=archkeep-e2e@invalid",
      "-c",
...
/**
 * Creates a native consumer workspace: two Go projects, `archkeep.json`
 ...
 * with a dependency chain `app → api → core`, `archkeep.json` at the root,
 ...
 * `archkeep.json`, with source files in the given language, no `nx`
 * dependency. Proves that Archkeep's analysis, graph construction, and
