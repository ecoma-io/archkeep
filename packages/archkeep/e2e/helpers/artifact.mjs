// Packs the `@ecoma-io/archkeep` artifact once per E2E run.
//
...
/**
 * Packs the `@ecoma-io/archkeep` artifact and returns the tarball path.
 *
 * @param {string} [packageDir] Absolute path to the package directory.
 *   Defaults to `packages/archkeep` relative to the repository root.
...
export function packArtifact(packageDir) {
  const pkgDir = packageDir ?? resolve(root, "packages/archkeep");
  const manifest = JSON.parse(readFileSync(join(pkgDir, "package.json"), "utf8"));
  const rootManifest = JSON.parse(readFileSync(join(root, "package.json"), "utf8"));

  const packDir = realpathSync(mkdtempSync(join(tmpdir(), "archkeep-e2e-pack-")));
