...
// contract an installed Archkeep reads (`src/architecture-intent/model.mjs`);
...
describe("drift", () => {
  it("exits 0 on a matching tree and states the intent fingerprint and row count", () => {
    const result = archkeep(matching.root, ["drift"]);
  ...
  it("reports a forbidden edge as dependencyForbidden, naming the pair", () => {
    const result = archkeep(forbidden.root, ["drift"]);
  ...
  it("produces a valid JSON envelope with drift findings", () => {
    const result = archkeep(forbidden.root, ["drift", "--format", "json"]);
    expect(result.exitCode).toBe(0);
  ...
  it("reports a missing required project as projectMissing, and check exits 1 on it", () => {
    const driftResult = archkeep(missing.root, ["drift"]);
    ...
    const checkResult = archkeep(missing.root, ["check"]);
  ...
  it("exits 3 on a malformed intent, and check does too", () => {
    const driftResult = archkeep(malformed.root, ["drift"]);
    expect(driftResult.exitCode).toBe(3);
    expect(driftResult.stderr).toMatch(/intent/);

    const checkResult = archkeep(malformed.root, ["check"]);
