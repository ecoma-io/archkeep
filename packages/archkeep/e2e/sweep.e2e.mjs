...
function freshHistoryDir() {
  return mkdtempSync(join(tmpdir(), "archkeep-e2e-sweep-hist-"));
}
...
function writeBaseline(dir) {
  ...
  const result = archkeep(clean.root, ["graph", "--format", "json"]);
...
function assertDeterministic(root, args, options = {}) {
  const first = archkeep(root, args);
  const second = archkeep(root, args);
...
describe("determinism sweep", () => {
  ...
  it("health <history-dir> --format json is byte-identical over a fixed history dir", () => {
    ...
    try {
      const capture = archkeep(clean.root, ["history", dir, "--capture"]);
  ...
  it("debt <history-dir> is byte-identical except the disclosed sampleTime", () => {
    ...
    try {
      ...
      const result = archkeep(clean.root, ["debt", dir, "--format", "json"]);
  ...
  it("waivers --format json is byte-identical except the disclosed remainingMs", () => {
    ...
    const result = archkeep(cleanWaiver.root, ["waivers", "--format", "json"]);
  ...
  it("check --format json is byte-identical on a tree with no workspace", () => {
    ...
    const nowhere = mkdtempSync(join(tmpdir(), "archkeep-e2e-sweep-nowhere-"));
    try {
      const first = archkeep(nowhere, ["check", "--format", "json"]);
      const second = archkeep(nowhere, ["check", "--format", "json"]);
...
describe("determinism sweep: profile-selected", () => {
  it("check --format json is byte-identical on a profile-selected Nx tree, exit 0", () => {
    ...
    const result = archkeep(profile.root, ["check", "--format", "json"]);
    expect(result.exitCode).toBe(0);
