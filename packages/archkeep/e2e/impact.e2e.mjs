...
describe("impact", () => {
  it("reports direct and transitive dependents for a leaf project", () => {
    ...
    const result = archkeep(monorepoConsumer.root, ["impact", "core", "--format", "json"]);
  ...
  it("reports only direct dependents for a mid-chain project", () => {
    ...
    const result = archkeep(monorepoConsumer.root, ["impact", "api", "--format", "json"]);
    expect(result.exitCode).toBe(0);
  ...
  it("reports empty dependents for a root project", () => {
    ...
    const result = archkeep(monorepoConsumer.root, ["impact", "app", "--format", "json"]);
    expect(result.exitCode).toBe(0);
  ...
  it("exits 2 for an unknown project name", () => {
    ...
    // not 3 (error).
    const result = archkeep(monorepoConsumer.root, ["impact", "nonexistent"]);
    expect(result.exitCode).toBe(2);
  });

  it("exits 2 for an unknown project name in JSON format", () => {
    const result = archkeep(monorepoConsumer.root, ["impact", "nonexistent", "--format", "json"]);
    expect(result.exitCode).toBe(2);
  });
});
...
describe("impact --config", () => {
  it("impact core on monorepo shows constraint context", () => {
    // The monorepo declares boundaryConfig in archkeep.json, so the
    ...
    const result = archkeep(monorepoConsumer.root, ["impact", "core"]);
    expect(result.exitCode).toBe(0);
  ...
  it("impact app on monorepo shows explicit empty message when no dependents", () => {
    ...
    const result = archkeep(monorepoConsumer.root, ["impact", "app"]);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("Constraint context");
