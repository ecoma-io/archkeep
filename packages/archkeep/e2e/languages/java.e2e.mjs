// E2E scenarios for the Java language fixture.
//
// Proves that Archkeep discovers Java imports, builds the correct graph,
...
import { JAVA_VIOLATION } from "../fixtures/languages/violations.mjs";
...
beforeAll(() => {
  ...
  consumer = createNativeLanguageConsumer(artifact, "java");
});
...
describe("Java language E2E", () => {
  it("Java dependency imports are discovered in the graph", () => {
  ...
  it("Java valid architecture passes check", () => {
  ...
  it("Java architecture violation is enforced", () => {
    const violator = createNativeLanguageConsumer(artifact, "java");
    try {
      commitFiles(violator.root, JAVA_VIOLATION, "domain reaches up into application");
      const result = archkeep(violator.root, ["check"]);
      expect(result.exitCode).toBe(1);
      const output = `${result.stdout}${result.stderr}`;
      expect(output).toContain("onlyTagsConstraintViolation");
      expect(output).toMatch(
        /libs\/domain\/src\/main\/java\/com\/example\/domain\/Violate\.java:\d+:\d+/,
      );
    } finally {
      violator.cleanup();
    }
  ...
  it("Java check produces a valid JSON envelope on a clean tree", () => {
  ...
  it("Java graph JSON includes expected projects and edges", () => {
    const result = archkeep(consumer.root, ["graph", "--format", "json"]);
  ...
  it("Java output is deterministic across two runs", () => {
