...
describe("a declared fitness function, end to end", () => {
  ...
  it("carries the failure into the JSON envelope with the edge that caused it", () => {
    const result = archkeep(consumer.root, ["fitness", "--format", "json"]);
    expect(result.exitCode).toBe(1);
  ...
  it("goes green again once the coupling is removed", () => {
    ...
    expect(archkeep(consumer.root, ["fitness"]).exitCode).toBe(0);
    expect(archkeep(consumer.root, ["check"]).exitCode).toBe(0);
  });
  ...
  it("refuses — never passes — when a matched project sits in no partition", () => {
    ...
    commitFiles(
      consumer.root,
      { "archkeep.json": sliceModel({ catalogTags: ["layer:slice"] }) },
    ...
    try {
      ...
      expect(archkeep(consumer.root, ["check"]).exitCode).toBe(3);
    } finally {
      commitFiles(consumer.root, { "archkeep.json": sliceModel() }, "catalog regains its tag");
    }
  });
});
