...
describe("explain", () => {
  ...
  it("exits 2 for a malformed site string", () => {
    const result = archkeep(nativeConsumer.root, ["explain", "bad-format"]);
    expect(result.exitCode).toBe(2);
  });

  it("exits 3 for a site with no import at that position", () => {
    const result = archkeep(nativeConsumer.root, ["explain", "libs/app/app.go:1:1"]);
  ...
  it("exits 3 for a nonexistent file site", () => {
    const result = archkeep(nativeConsumer.root, ["explain", "libs/app/nonexistent.go:1:1"]);
    expect(result.exitCode).toBe(3);
  });
});
