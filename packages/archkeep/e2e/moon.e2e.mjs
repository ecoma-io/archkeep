...
// files. Archkeep discovers the workspace via `.moon/` and reads the project
// graph from `moon project-graph --json` — the same one-call contract as the
// Nx provider, but without Nx installed. These scenarios prove that Archkeep's
...
describe("Moon", () => {
  ...
  it("exits 0 on a clean Moon tree and states what it inspected", () => {
    const result = archkeep(moonConsumer.root, ["check"]);
  ...
  it("exits 0 on a clean Moon tree for graph and states project and edge counts", () => {
    const result = archkeep(moonConsumer.root, ["graph"]);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toMatch(/[1-9]\d* project/);
  ...
  // of Archkeep's word; while the provider matched the two by spelling, this
  ...
  it("exits 1 on a dependency declared only in moon.yml, with no import behind it", () => {
    ...
    try {
      ...
      // tree is refused outright for carrying `archkeep.json`
      ...
      // difference.
      const json = archkeep(violator.root, ["check", "--format", "json"]);
  ...
  // all: with the mapping right they carry Archkeep's `implicit` type and
  ...
  it("judges the fixture's permitted hand-declared edges AS declarations, and reports none", () => {
    const result = archkeep(moonConsumer.root, ["check", "--format", "json"]);
    expect(result.exitCode).toBe(0);
    ...
    const graph = archkeep(moonConsumer.root, ["graph", "--format", "json"]);
  ...
  it("produces a valid JSON envelope for graph on a clean Moon tree", () => {
    const result = archkeep(moonConsumer.root, ["graph", "--format", "json"]);
    expect(result.exitCode).toBe(0);
    expect(result.json).not.toBeNull();
  ...
  it("diff exits 0 and reports no changes against a self-baseline", () => {
    ...
    archkeep(moonConsumer.root, ["graph", "--format", "json", "--output", baselineFile]);
    const result = archkeep(moonConsumer.root, ["diff", baselineFile]);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("no changes");
  });

  it("impact reports direct and transitive dependents for a leaf project", () => {
    const result = archkeep(moonConsumer.root, ["impact", "core", "--format", "json"]);
    expect(result.exitCode).toBe(0);
    expect(result.json).not.toBeNull();
  ...
  it("explain explains a violating import site", () => {
    ...
    try {
      ...
      const result = archkeep(violator.root, [
        "explain",
  ...
  it("uses the Moon provider — provider is moon, not native or nx", () => {
    // The fixture has `.moon/` but no `archkeep.json` and no `nx.json`, so
  ...
  it("context returns project tags and constraints on a Moon tree", () => {
    const result = archkeep(moonConsumer.root, ["context", "core", "--format", "json"]);
    expect(result.exitCode).toBe(0);
    expect(result.json).not.toBeNull();
