...
function freshHistoryDir() {
  return mkdtempSync(join(tmpdir(), "archkeep-e2e-history-"));
}
...
describe("history capture determinism (E-F05)", () => {
  it("capturing twice into the SAME dir differs only in the duplicate marker", () => {
    ...
    try {
      const first = archkeep(nativeConsumer.root, [
        "history",
        dir,
        "--capture",
        "--format",
      ...
      const second = archkeep(nativeConsumer.root, [
        "history",
        dir,
        "--capture",
        "--format",
  ...
  it("capturing once into two EMPTY dirs is byte-identical", () => {
    ...
    try {
      const a = archkeep(nativeConsumer.root, ["history", dirA, "--capture", "--format", "json"]);
      const b = archkeep(nativeConsumer.root, ["history", dirB, "--capture", "--format", "json"]);
...
describe("history (empty and capture)", () => {
  it("exits 3 when the directory holds no snapshots — never a record of nothing", () => {
    ...
    const dir = freshHistoryDir();
    try {
      const result = archkeep(nativeConsumer.root, ["history", dir]);
  ...
  it("captures a snapshot into an empty directory", () => {
    const dir = freshHistoryDir();
    try {
      const result = archkeep(nativeConsumer.root, ["history", dir, "--capture"]);
  ...
  it("captures a snapshot, then deduplicates when nothing changed", () => {
    const dir = freshHistoryDir();
    try {
      const first = archkeep(nativeConsumer.root, ["history", dir, "--capture"]);
      expect(first.exitCode).toBe(0);
      ...
      // transition.
      const second = archkeep(nativeConsumer.root, ["history", dir, "--capture"]);
      expect(second.exitCode).toBe(0);
  ...
  it("describes a captured history as JSON with the versioned envelope", () => {
    const dir = freshHistoryDir();
    try {
      archkeep(nativeConsumer.root, ["history", dir, "--capture"]);
      const result = archkeep(nativeConsumer.root, ["history", dir, "--format", "json"]);
      expect(result.exitCode).toBe(0);
...
describe("history (evolution)", () => {
  it("records an architectural change as a transition between snapshots", () => {
    ...
    const dir = freshHistoryDir();
    try {
      archkeep(consumer.root, ["history", dir, "--capture"]);
      ...
      const result = archkeep(consumer.root, ["history", dir, "--capture"]);
      expect(result.exitCode).toBe(0);
      ...
      const describe = archkeep(consumer.root, ["history", dir, "--format", "json"]);
