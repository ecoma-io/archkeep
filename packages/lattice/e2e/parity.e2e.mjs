...
describe("parity (full)", () => {
  ...
  it("check agrees on violation rule ID and file when violating", () => {
    ...
    try {
      ...
      const nativeResult = lattice(nativeViolator.root, ["check", "--format", "json"]);
      const nxResult = lattice(nxViolator.root, ["check", "--format", "json"]);
  ...
  it("check JSON envelope agrees on command, schemaVersion, status, exitCode", () => {
    const nativeResult = lattice(nativeConsumer.root, ["check", "--format", "json"]);
    const nxResult = lattice(nxConsumer.root, ["check", "--format", "json"]);
