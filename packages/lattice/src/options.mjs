 ...
 *
 * Two filenames — plus the intent config, which the drift command reads — and
 * nothing else. All three are Nx CONVENTIONS rather than fixed
 ...
 * says so at best, reads a stale one at worst. The intent filename has the same
 * shape of consequence: a drift gate that reads the wrong file reads no law,
 * and reports the tree clean against nothing. That is the difference between
 * this trio and the manifests next door: `go.mod`, `Cargo.toml`,
...
export const DEFAULT_OPTIONS = Object.freeze({
  ...
  /** The workspace's intended architecture — the drift command's contract. */
 ...
 * @returns {{boundaryConfig: string, tsConfig: string, intentConfig: string}}
