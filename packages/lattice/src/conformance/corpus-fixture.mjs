 ...
 * (`lattice.json`) tree per case, each in its own throwaway root.
 *
 ...
 * project model from the `lattice.json` at the root it is handed
 ...
 * sources and its manifests. It never writes `lattice.json`, never repeats the
  ...
  for (const spec of cases) {
    for (const project of spec.projects) {
      if ((project.tags ?? []).includes(DENY_ALL_TAG)) {
        throw new Error(
          `lattice corpus: case '${spec.id}' tags project '${project.name}' with ` +
...
/** The `lattice.json` a case's projects and declared files add up to. */
function nativeModel(spec) {
  ...
  // (`../providers/native/coverage.mjs`), so `lattice.json` and
...
export function materializeCase(spec) {
  ...
  const reserved = ["lattice.json", BOUNDARY_CONFIG_FILE, DENY_ALL_CONFIG_FILE, TS_CONFIG_FILE];
  const shadowed = Object.keys(spec.files).filter((relative) => reserved.includes(relative));
  if (shadowed.length > 0) {
    throw new Error(
      `lattice corpus: case '${spec.id}' declares ${shadowed.join(", ")}, which this module ` +
  ...
  const root = mkdtempSync(join(realpathSync(tmpdir()), `lattice-corpus-${spec.id}-`));
  ...
  put("lattice.json", `${JSON.stringify(nativeModel(spec), null, 2)}\n`);
