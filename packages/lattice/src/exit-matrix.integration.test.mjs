...
const NX_JSON = `${JSON.stringify({
  plugins: [
    { plugin: "@ecoma-io/lattice/nx", options: { boundaryConfig: "module-boundaries.config.mjs" } },
  ],
})}\n`;
...
const world = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-"));
const brokenRoot = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-broken-"));
const incompleteRoot = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-incomplete-"));
const commitlessRoot = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-commitless-"));
const artifactsDir = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-artifacts-"));
const histDir = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-hist-"));
const emptyHistDir = mkdtempSync(join(tmpdir(), "lattice-exit-matrix-hist-empty-"));
afterAll(() => {
  for (const dir of [
    world,
    brokenRoot,
