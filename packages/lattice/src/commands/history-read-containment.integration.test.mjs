...
// `readdirSync` the same directory unguarded — a `.lattice/history ->
...
describe("history read containment (G-10) — real filesystem", () => {
  ...
  beforeAll(() => {
    root = realpathSync(mkdtempSync(join(tmpdir(), "lattice-history-read-")));
    outside = realpathSync(mkdtempSync(join(tmpdir(), "lattice-history-read-outside-")));
    ...
    symlinkSync(join(outside, "history"), join(root, ".lattice", "history"));
  });
  ...
  it("refuses a history directory whose realpath leaves the workspace, loudly", () => {
    expect(() => readSnapshots(join(root, ".lattice", "history"), root)).toThrow(
