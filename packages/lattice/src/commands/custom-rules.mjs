 ...
 * runs `lattice_describe` inside a worker — and paying for it to reach a
...
function refuseLoad(name, reason) {
  ...
  throw new Error(`lattice: custom rule "${name}": ${reason}`);
}
...
export async function customRulesForCheck(
  ...
  for (const row of rows) {
    ...
    if (!outcome.ok) throw new Error(`lattice: ${outcome.failure.reason}`);
