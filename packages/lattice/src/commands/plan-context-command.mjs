 ...
 * Lattice-governed workspace.
 *
 * ## The boundary this module holds
 *
 * Lattice produces deterministic architecture facts and constraints; an agent
...
export async function planContextCommand(projectName, paths, commandContext, config) {
  ...
  if (provider === "nx" && !pluginGap.registered && pluginGap.manifests.length > 0) {
    throw new Error(
      `lattice: refusing to build a planning context for an Nx workspace where this plugin is ` +
  ...
  if (paths.length > 0 && affected.length === 0) {
    notes.push(
      `no path matched a project-owned file (given: ${paths.join(", ")}), so the scope fell ` +
        `back to the whole workspace — re-check the path spellings against 'lattice graph'.`,
    );
  }
  ...
  const result = {
    ...
    plan: {
      ...
      verify: [
        ...
        `lattice impact ${projectName} --format json`,
