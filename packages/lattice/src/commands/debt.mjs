 ...
 * `lattice.json` policy — so a debt run and a `check` run never disagree about
...
export async function debtCommand(dir, commandContext, options = {}) {
  ...
  if (notAnalyzed.length > 0) {
    throw new Error(
      `lattice: debt has incomplete coverage — ${notAnalyzed.length} file` +
  ...
  if (intent === undefined) {
    throw new Error(
      `lattice: debt requires a tracked ${INTENT_FILE} at the workspace root, but none ` +
  ...
  if (verdict.unresolved.length > 0) {
    throw new Error(
      `lattice: cannot build a debt ledger — ${INTENT_FILE} cannot be verified: ` +
        verdict.unresolved
