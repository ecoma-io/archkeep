 ...
 * reuse the same Nx-marker and `lattice.json`-marker fixture-tree builders
...
/**
 * Writes the native half of the `simple` pair: `lattice.json` declaring the
  ...
  write(
    ...
    JSON.stringify({
      projects: {
        ...
        // an Nx tree finds them with no `lattice.json` at all — an absent
