/**
 * The native project-model provider: `lattice.json`, no Nx installed, no
 ...
 * `nx` installed, must still be able to discover its `lattice.json` projects
 * without paying for a compiler it will never call.
 */
import { projectOwning } from "../../analysis/source-util.mjs";
import { LATTICE_MODEL_FILE, loadNativeModel } from "./model.mjs";
...
export function discover({ root, files, readFile }) {
  ...
  if (coverage.stale.length > 0) {
    throw new Error(
      `lattice: ${root}/${LATTICE_MODEL_FILE} describes a workspace that does not match the ` +
        `tree:\n  ` +
        coverage.stale
          .map(
            (row) =>
 ...
 * workspace root carries `lattice.json` rather than `nx.json` (`../nx.mjs`'s
