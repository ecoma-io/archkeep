...
use lattice_rule_sdk::harness::{expect_verdict, replay};
...
fn the_verdict_bytes_are_the_ones_that_cross_the_abi() {
    ...
    assert_eq!(
        rule::lattice_evaluate_json(&bundle),
...
fn every_finding_the_fixtures_produce_is_in_the_catalogue() {
    let declared: Vec<&str> = rule::LATTICE_RULE
        .findings
        .iter()
        .map(|entry| entry.id())
        .collect();
    ...
    for fixture in [
        ...
        for finding in replay(fixture, rule::lattice_evaluate_json)["findings"]
            .as_array()
