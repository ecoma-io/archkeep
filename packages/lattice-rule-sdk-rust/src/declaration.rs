...
//! (`../../lattice/src/custom-rules/host.mjs`, `describeViolation` and
...
impl CatalogueEntry {
    ...
    /// end to end (`../../lattice/src/config.mjs`, `CUSTOM_RULE_NAME_PATTERN`).
...
/// Everything `lattice_describe` answers with.
///
/// Built by the `lattice_rule!` macro from the author's literals; there is no
...
impl RuleDeclaration {
    ...
    /// (`../../lattice/src/custom-rules/host.mjs`) moved to the earliest place
    ...
    pub const fn assert_valid(&self) {
        assert!(
            ...
            "lattice_rule!: `name` must be lowercase letters and digits joined by single dashes, \
             the same grammar the policy row's `name` is held to"
        );
        assert!(
            !self.findings.is_empty(),
            "lattice_rule!: `findings` must declare at least one entry — a rule that can name \
        ...
        while index < self.findings.len() {
            assert!(
                ...
                "lattice_rule!: every finding id must be lowercase letters and digits joined by \
                 single dashes"
            );
            assert!(
                !self.findings[index].message.is_empty(),
                "lattice_rule!: every finding needs a message — a catalogue entry with none \
            ...
            while other < index {
                assert!(
                    ...
                    "lattice_rule!: two findings share one id — a report would resolve findings \
...
/// The document `lattice_describe` returns, in the shape the host parses.
///
...
impl RuleDeclaration {
    /// This declaration as the UTF-8 JSON text `lattice_describe` answers with.
    ///
    /// # Panics
    ///
