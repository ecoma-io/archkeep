...
# subset it reads in `LATTICE_RULE["needs"]`; an entry outside this tuple makes
...
class Policy:
    ...
    (`../../../lattice/src/config.mjs`), and a second model of it here would be
    ...
    def _declared(self, kind, attribute):
        if kind not in self.needs:
            raise UndeclaredEvidence(
                ...
                "LATTICE_RULE['needs'] so the engine hands it over. A rule is given the kinds "
