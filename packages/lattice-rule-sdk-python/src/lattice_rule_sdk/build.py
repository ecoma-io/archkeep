...
`lattice_rule!` macro. This file checks only that the pieces are there
(`LATTICE_RULE` exists and states three keys, `evaluate` is callable), because
...
CRATE_NAME = "lattice_rule_carrier"
...
def read_declaration(rule_path: Path):
    """The rule's `LATTICE_RULE` declaration and its `evaluate`, from its source.
    ...
    spec = importlib.util.spec_from_file_location("lattice_rule_under_build", rule_path)
    ...
    declaration = getattr(module, "LATTICE_RULE", None)
    if not isinstance(declaration, dict):
        raise BuildError(
            f"{rule_path} states no LATTICE_RULE mapping — a rule declares its name, the "
    ...
    if missing:
        raise BuildError(
            f"{rule_path}'s LATTICE_RULE states no {', '.join(missing)} — the three keys are "
            f"what the artifact answers lattice_describe with"
        )
...
def render_carrier(declaration, rule_path: Path, crate_dir: Path, sdk_dependency: str) -> None:
    ...
    # `lattice_rule!` macro takes; `{{NEEDS_WIRE}}` is the same kinds as Rust
    ...
    shutil.copyfile(Path(__file__).resolve().parent / "runtime.py", source_dir / "lattice_rule_sdk.py")
    shutil.copyfile(rule_path, source_dir / "rule.py")


def _findings_literal(findings) -> str:
    """The catalogue as the Rust literal list `lattice_rule!` takes.
    ...
    for index, entry in enumerate(findings):
        try:
            ...
            raise BuildError(
                f"LATTICE_RULE['findings'][{index}] is {entry!r}, and a catalogue entry states "
...
def run_cargo(crate_dir: Path, offline: bool) -> Path:
    ...
    if completed.returncode != 0:
        raise BuildError(
            ...
            "LATTICE_RULE, assert_valid or a const evaluation, it is the rule's declaration "
...
def build(rule_path: Path, out_path: Path, sdk_dependency: str, keep: Path | None, offline: bool) -> None:
    ...
    if keep is not None:
        ...
        return
    with tempfile.TemporaryDirectory(prefix="lattice-rule-carrier-") as scratch:
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m lattice_rule_sdk.build",
        description="Compile a Python Lattice rule into a core-WebAssembly artifact.",
    )
...
def _own_version() -> str:
    ...
    try:
        ...
        return version("lattice-rule-sdk")
