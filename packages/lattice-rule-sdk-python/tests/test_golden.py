...
copies of `../../lattice-rule-sdk-rust/fixtures/` — the same files, checked by
...
# them importable.
from lattice_rule_sdk import drive
...
def replay(name):
    ...
    return drive(rule.evaluate, bundle, rule.LATTICE_RULE["needs"])
...
class TheGoldenFixtures(unittest.TestCase):
    ...
    def test_every_finding_the_fixtures_produce_is_in_the_catalogue(self):
        ...
        """
        declared = {entry["id"] for entry in rule.LATTICE_RULE["findings"]}
        seen = 0
