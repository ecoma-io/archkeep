...
It is the Python spelling of `../../lattice-rule-sdk-rust/examples/forbidden_tag_dependency.rs`,
...
from lattice_rule_sdk import Finding, from_findings, not_applicable, unknown
...
#: The declaration the carrier turns into `lattice_describe`'s answer.
#:
#: It sits beside the function it describes, the way the Rust binding's
#: `lattice_rule!` does, so a finding id and the code that emits it can never be
