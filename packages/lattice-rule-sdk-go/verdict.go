...
// (../lattice/src/custom-rules/host.mjs, verdictViolation). Every one of those
...
//     (../lattice-rule-sdk-rust/src/verdict.rs).
...
// is (../lattice/src/analysis/contract.md), silently dropping the position
...
func (f Finding) At(sourceFile string, line, column uint32) Finding {
	if line == 0 || column == 0 {
		panic(fmt.Sprintf(
			"latticerule: Finding.At(%q, %d, %d): positions are 1-based integers, and a finding at "+
