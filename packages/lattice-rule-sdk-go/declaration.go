...
// not the one the policy row declared (../lattice/src/custom-rules/host.mjs,
...
// build` (../lattice-rule-sdk-rust/src/declaration.rs). Go has no const
...
//
// The roster is the engine's (../lattice/src/custom-rules/evidence.mjs,
...
type CatalogueEntry struct {
	...
	// (../lattice/src/config.mjs, CUSTOM_RULE_NAME_PATTERN).
	ID string `json:"id"`
...
// rule by the contract's own shape — lattice_describe answers with a single
...
// the host reads lattice_describe before anything else exists
// (../lattice/src/custom-rules/host.mjs, loadCustomRule). The ABI shell
...
// and carried on would answer lattice_describe with a document naming a rule
// that does not exist. Off wasm the panic is what an author reads from `go
// test`; on wasm it is a trap the host names (../lattice/src/custom-rules/host.mjs,
// outcomeReason's "trap" arm), which is a load failure rather than a rule that
// quietly reports nothing.
func Register(rule Rule) {
	if registered != nil {
		panic(fmt.Sprintf(
			"latticerule.Register: this artifact already declares the rule %q — one artifact is "+
				"one rule, because lattice_describe answers with a single document",
			registered.Name))
	}
	if violation := validate(rule); violation != "" {
		panic("latticerule.Register: " + violation)
	}
	registered = &rule
}
...
// only one available: every document lattice_describe could answer with instead
...
func Registered() Rule {
	if registered == nil {
		panic("latticerule.Registered: no rule was registered — call latticerule.Register from an " +
			"init function, which is what runs before the host reads lattice_describe")
	}
	return *registered
}
...
// describe is the document lattice_describe returns, in the shape the host
// parses.
//
...
//
// It is the pure half of lattice_describe: target-independent, so a `go test`
...
// binding (../lattice-rule-sdk-rust/src/verdict.rs) over the same verdict, and
...
func marshalJSON(document any) string {
	...
	if err := encoder.Encode(document); err != nil {
		panic("latticerule: a document of strings and integers failed to serialize: " + err.Error())
	}
