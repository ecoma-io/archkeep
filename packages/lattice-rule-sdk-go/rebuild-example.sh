...
# script does not do and this one must (../lattice-rule-sdk-rust/rebuild-example.sh).
...
# ../lattice/ would be the dependency direction the scope axis in the
...
const packed = BigInt.asUintN(64, instance.exports.lattice_describe());
...
if (pointer + length > memory.byteLength || length === 0) {
  throw new Error(`${path}: lattice_describe returned the range ${pointer}..${pointer + length}`);
}
