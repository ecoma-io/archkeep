...
// (../lattice/src/custom-rules/host.mjs). A rule therefore has no lifetime
...
// names (../lattice/src/custom-rules/host.mjs, outcomeReason's "trap" arm) —
// the loud direction, rather than a pointer the host would write through into
// nothing.
func alloc(size int32) int32 {
	if size < 0 {
		panic("lattice_alloc: the host asks a non-negative length")
	}
...
func evidenceSlice(ptr, length int32) []byte {
	if ptr < 0 || length < 0 {
		panic("lattice_evaluate: a negative pointer or length is not a range")
	}
	memoryBytes := uint64(uint32(wasmMemorySize(0))) * wasmPageSize
	if uint64(uint32(ptr))+uint64(uint32(length)) > memoryBytes {
		panic("lattice_evaluate: the evidence range reaches past this module's own linear memory")
	}
...
//export lattice_alloc
func latticeAlloc(size int32) int32 {
	ensureRuntime()
	return alloc(size)
}

//export lattice_describe
func latticeDescribe() int64 {
	ensureRuntime()
	return pack(DescribeJSON())
}

//export lattice_evaluate
func latticeEvaluate(ptr, length int32) int64 {
	ensureRuntime()
