...
docker run --rm \
  ...
  cargo build --release --locked --target wasm32-unknown-unknown \
    --example tag_cardinality --example forbidden_tag_combination
  tar -C target/wasm32-unknown-unknown/release/examples -cf - \
    tag_cardinality.wasm forbidden_tag_combination.wasm
' > "$archive"
...
for name in tag-cardinality forbidden-tag-combination; do
