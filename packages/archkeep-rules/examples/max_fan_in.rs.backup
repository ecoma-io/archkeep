...
//!
//! - `fail` — an in-scope project is depended on by more distinct projects than the
...
//!   "sha256": "<the contents of rules/max-fan-in.wasm.sha256>",
//!   "params": {
//!     "max": 2,
//!     "match": ["scope:shared"]
//!   },
...
use archkeep_rules::topology::fan_in;
...
fn evaluate(evidence: &Evidence) -> Verdict {
    ...
    // Run the topology evaluator to compute per-project fan-in counts.
    ...
    let fan_ins = match fan_in(&evidence.model.projects, &evidence.graph.edges) {
        Ok(counts) => counts,
        Err(e) => return Verdict::unknown(e),
    };
    ...
    for project in &in_scope {
        // Find the fan-in count for this project (the evaluator preserves model order)
        let count = fan_ins
            .iter()
        ...
        if count > max as usize {
            findings.push(
                Finding::new(
                    ...
                    format!(
                        "{} is depended on by {} distinct projects, above the budget of {}",
...
archkeep_rule! {
    ...
    findings: [
        (
            "fan-in-budget-exceeded",
            "a project is depended on by more distinct projects than the declared budget"
        ),
    ],
    evaluate: evaluate,
}
