...
//
// The Rust SDK generates its exports from `lattice_rule! { … }`
// (`../../lattice-rule-sdk-rust/src/lib.rs`). AssemblyScript has no macro
...
// `describePacked` and `evaluatePacked` below are for, and let `lattice_alloc`
// be re-exported verbatim (`export { lattice_alloc } from "…/assembly";` in the
 ...
 *
 * Re-exported by a rule's entry file as `lattice_alloc`, which is the ABI name.
 ...
 * (`../../lattice/src/custom-rules/host.mjs`, the `trap` arm) — the loud
 * direction, rather than a pointer the host would write through into nothing.
 */
export function lattice_alloc(len: i32): i32 {
 ...
 *
 * One line inside an author's `lattice_describe`.
 */
 ...
 * `lattice_alloc` answered with, and the host has already refused a range
