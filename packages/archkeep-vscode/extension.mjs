...
/** The `archkeep.server.path` setting for one folder, or "". */
function configuredServerPath(folder) {
  return workspace.getConfiguration("archkeep", folder).get("server.path", "").trim();
}
...
async function openSession(folder) {
  ...
  item.command = { command: "archkeep.showLog", title: "Show log" };
...
export async function activate(context) {
  output = window.createOutputChannel("Archkeep", { log: true });

  context.subscriptions.push(
    output,
    commands.registerCommand("archkeep.restart", restart),
    commands.registerCommand("archkeep.showLog", () => output?.show(true)),
    workspace.onDidChangeWorkspaceFolders(syncFolders),
    workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("archkeep.server.path")) {
        return restart();
      }
    }),
  );
