...
export const SERVER_PACKAGE = "@ecoma-io/archkeep";
 ...
 * @param {string} [input.configuredPath] the `archkeep.server.path` setting
...
export function locateServer({ nxRoot, configuredPath = "", exists, resolvePackageJson }) {
  ...
  if (configuredPath.trim() !== "") {
    ...
    return {
      ...
      source: "setting",
      searched,
      reason: `archkeep.server.path is set to ${configuredPath}, and there is no file at ${configured}.`,
    };
  }
  ...
  return {
    ...
    source: null,
    searched,
    reason: `${SERVER_PACKAGE} is not installed in ${nxRoot}, so nothing is checking module boundaries in this window. Add it to the workspace, or set archkeep.server.path to an lsp.mjs.`,
  };
}
