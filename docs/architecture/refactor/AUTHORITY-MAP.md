...
**Descriptive commands project canonical evaluation; they never re-evaluate.**
This is the general law DG-4 asked for, stated here once and closed by
[PD-18](DECISIONS.md#program-decisions): every verb that answers a question
about the architecture obtains its content from the lane's own engines or the
canonical snapshots — never from a second evaluation. The capability facades
(`src/commands/<word>-capability.mjs`) make the same fact structural: a
re-export roster has nowhere to hide a judgment.
...
| capability facades (`src/commands/<word>-capability.mjs`) | nothing — explicit re-export rosters of their word's verbs, by construction ([PD-18](DECISIONS.md#program-decisions))               | gain logic, defaults, or a second evaluation path                                          | zero judgment by construction                                                                                        |
