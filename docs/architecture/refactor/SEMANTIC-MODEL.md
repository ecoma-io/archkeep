...
| Catalog-integrity verdict (`rules verify`) | the verify fold — `src/commands/rules.mjs:445-448` (status), `cli.mjs:2017-2019` (exit)                                                                         | `jsonEnvelope` + `EXIT_FOR_STATUS` (the shared contract); no lane module consumes it                                 | bounded integrity surface — a different law from the lane ([PD-8](DECISIONS.md#program-decisions)) |
