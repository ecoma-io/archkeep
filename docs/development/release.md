...
third state, not a failure. As measured on 2026-09-11:
...
| a workspace outside this repo gates a build on `check` | **met**     | `gate-attestation.json` (`ecoma-io/action-agents@0.27.0`, commit `a842c23`) and `archkeep-attestation.json` (`ecoma-io/loom@0.28.0`, commit `ef79c56`) published at the consumers' repo roots and accepted by the verifier; each names a revision whose push CI ran the boundary step green (action-agents run 34367745437, loom run 34506048544), and the gate is recorded biting in both directions at earlier revisions (action-agents red run 34367509931 recovered green 34367745437; loom's `0.24.0` pair, red 34368289160 recovered green 34368776193); `check-readiness --attestations` reads the row met naming both |
| a quiet stretch in what an unchanged workspace is told | **not met** | `pnpm readiness` reads "5 of 50 commits" since #895 (2026-09-11); #895 touched the output-contract surface without a marker, as did #838, #832, and #827 before it, and the stretch counts quiet commits from the tip — it moves only by landing without verdict-surface changes                                                                                                                                                                                                                                                                                                                                              |
| releases land without a hand on them                   | **met**     | the 24 consecutive tags after the never-published 0.12.0 each carry a published npm version; the 0.27.0, 0.27.1, and 0.28.1 release runs succeeded at `run_attempt: 1` with self-started gates (0.28.1's lane: run 34578132834), and each GitHub release carries a `.vsix` whose internal version matches its tag (checked 2026-09-11)                                                                                                                                                                                                                                                                                        |

One row is actionable, and its currency is waiting: the quiet stretch needs
verdict-quiet commits to land on `main` — roughly 45 more at the 2026-09-11
