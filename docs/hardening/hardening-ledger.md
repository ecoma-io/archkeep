...
| F6  | `historyOutputRefusal` decides with exact `dirname === dir` string equality, so a symlinked alias of the history directory escapes the poisoning guard                                                                                                       | `packages/archkeep/src/commands/history.mjs:798-816`                                                             | deferred                                       | issue [#888](https://github.com/ecoma-io/archkeep/issues/888) — reproduced end to end with the real CLI                                                                                                                                                                             |
| F7  | provenance binding edges are built at O(decisions × bindings × rows), and provenance has no benchmark — the existing bench covers identity and evolution only                                                                                                | `packages/archkeep/src/governance/provenance-graph.mjs:343-351`, `packages/archkeep/e2e/bench/history-bench.mjs` | deferred                                       | issue [#889](https://github.com/ecoma-io/archkeep/issues/889) — measurement-first extension of the existing bench rather than adding a harness                                                                                                                                      |
...
a filed issue, a new measurement. F1–F4 moved at #883's merge into `942e7f9c`;
F5–F7 moved when issues #887, #888 and #889 were filed; F8 moved when the
probe was attempted on 0.28.0. None moved because a row was reworded.
