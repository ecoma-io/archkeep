...
`graph` never exits 1. Exit 1 belongs to the verdict-carrier roster — `check`, `fitness`, `delta`, `change` and `rules verify` ([docs/reference/exit-codes.md](../reference/exit-codes.md)); `graph` is descriptive.
