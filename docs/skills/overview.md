...
or proposal-only: `graph`, `diff`, `drift`, `discover`, `reconcile`, `impact`,
`explain`, `context`, `history`, `waivers`, `fitness`, `health`, `debt`,
`provenance`, and `adr` — all fifteen — never exit 1 on their own, and both
