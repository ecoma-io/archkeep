...
introduced violation is not covered by an active waiver, `"no-verdict"`
(exit `3`) when any item — violation or unresolvable record — could not be
classified, and `"ok"` (exit `0`) otherwise. Its refusals (unreadable or
...
| `summary`       | object                                                                     | Bucket counts: `{introduced, introducedWaived, resolved, unchanged, unknown, unresolvable: {introduced, resolved, unchanged, unknown}}`. `introducedWaived` counts introduced entries the current waiver table covers — reported, not gating; `introduced - introducedWaived > 0` is exactly what makes `status` `"findings"`. |
