...
| `unresolvedDecisionRefs` | `string[]` (optional)                                                                           | `decisionRef` values from `depConstraints` rows (`violations[].constraint.decisionRef`, and any declared-edge finding's) that resolve to no ADR, rule, or fitness record the workspace's `docs/adr/` registry knows — sorted, deduplicated. Present only when non-zero, the same `waived`-style bargain: an unchanged tree's envelope is unchanged. A documentation fact about the row's _citation_, never a finding of its own — it changes no byte of `status`/`exitCode` (`src/governance/adr-registry.mjs`'s `resolveDecisionRef`; `AGENTS.md`, "an empty result is a claim, not a shrug" — the empty case here means every citation resolves, or none exists). This top-level list covers only `depConstraints`/declared-edge rows; an unresolved citation on an INTENT row lives under `result.intent.unresolvedDecisionRefs` instead, and ITS exit-3 folding is the intent block's contract above, not this field's.                                                                                           |
