...
| `blindSpots`    | `{file, line, column, reason, dynamic?}[]` | Site-level failures: the file was analyzed, but one import site's target is not statically knowable. A row carrying `dynamic: true` is the declared non-literal-argument limit -- disclosed, exit-neutral. A row without it is an unresolvable literal specifier -- a concrete question the run could not answer, which on a findings-free run forces exit 3 (#595). |
...
and it lands in the same no-verdict lane unless it carries the `dynamic` marker
-- the language-declared limit static analysis cannot close.
For an unresolvable import the line is whether the specifier names a declared
project: a workspace-internal dependency that should resolve to a project node
but cannot is a whole-file failure (a missing edge, a coverage hole), while a
package import that names no declared project is a permanent blind spot -- one
that now withholds the verdict (#595), unlike its exit-neutral dynamic
counterpart.
