...
| 1    | findings -- boundary violations, declared-edge (`implicitDependencies`) violations, go.work drift, dead tsconfig path aliases, architecture-intent findings, a failing fitness function, a failing custom rule, or a non-waived violation `delta` classifies as introduced | `check`, `fitness`, and `delta`. A failing fitness function is a finding (D-09), so is a custom rule's `fail`, and so is a violation the compared change introduced without an active waiver covering it; every other command that finds something reports it but exits 0, so it never claims the findings exit code without a failed enforcement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
